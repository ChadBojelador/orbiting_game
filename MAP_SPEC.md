...
- Edit original-topology.ts / OriginalWorldMap and run npm run assets:original together. The generated original-data.ts is collision only. Independent renderer raycasts and spawn movement tests verify the bake. Boundary and paint remain cosmetic; the windmill blades stay at their baked pose. No freeze-tag behavior returns.
