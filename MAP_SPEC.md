...
- Original village cover is rendered as solid wooden blocks. Mesh terrain, bridge decks/rails, landmarks, vegetation, and cover share baked triangle collision. River ribbons provide shallow channel floors. Ocean swimming uses the existing buoyancy model at this map's sea level.
- The 3.125 m terrain grid is an indexed, welded, closed volume: top faces point upward, exposed coast/river/world-envelope walls point outward, and a shared underside closes each land section at `SEA_LEVEL - 0.55`. Terrain, paths, rivers, and ocean use normal front-face culling; only intentionally two-sided presentation sheets such as waterfalls and the boundary remain double-sided.
