...
test.beforeAll(async()=>{
  server=await startServer(readConfig({GUEST_SESSION_SIGNING_SECRET:'e2e-only-secret-at-least-32-characters',GAME_SERVER_PORT:'2568',COUNTDOWN_SECONDS:'1',CLIENT_ORIGIN:'http://localhost:5174',DEV_BOT_COUNT:'0'}),{isReady:async()=>true,saveMatchSummary:async()=>true,close:async()=>{}});
});
test.afterAll(async()=>{await server?.stop();});
async function identify(page:Page,name:string){
  await page.goto('/');await page.getByLabel('Display name').fill(name);await page.getByRole('button',{name:'Let’s go'}).click();
  await expect(page.getByRole('button',{name:'Create private room'})).toBeVisible();
}
async function create(page:Page){
  const response=page.waitForResponse(r=>r.url().endsWith('/api/rooms')&&r.request().method()==='POST');
...
test('desktop guests play, shoot, die, respawn, see scores and finish',async({page,browser})=>{
  ...
  await identify(page,'FPS Host');const {room,code}=await create(page);
  ...
  try{
    ...
    await page.keyboard.press('KeyR');await expect(page.getByText('Reloading…')).toBeVisible();
    await expect(page.getByText('Reloading…')).toHaveCount(0,{timeout:4000});
    await page.keyboard.press('Digit2');await expect(page.locator('.ammo-display')).toContainText('Snowmelt');
    await page.keyboard.down('Tab');await expect(page.getByRole('region',{name:'Scoreboard'})).toBeVisible();await page.keyboard.up('Tab');
    await page.screenshot({path:'test-results/fps-desktop.png'});
    a.kills=30;
    const advance=Reflect.get(room,'advance') as (now:number)=>void;advance.call(room,Date.now());room.broadcastPatch();
    ...
    await page.getByRole('button',{name:'Back to lobby'}).click();await expect(page.getByRole('button',{name:'Create private room'})).toBeVisible();
...
test('mobile supports simultaneous movement, look and fire without overflow',async({browser})=>{
  ...
  try{
    ...
    await page.getByLabel('Invite code',{exact:true}).fill('ABCDEFGH');await page.getByRole('button',{name:'Join room',exact:true}).tap();await expect(page.getByRole('alert')).toContainText('Room not found');
    await page.screenshot({path:'test-results/fps-lobby-mobile.png',fullPage:true});
    const {room}=await create(page);await start(page);
    ...
    await cdp.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[{id:1,x,y:y-40},{id:2,x:245,y:350}]});
    await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{id:1,x,y:y-40},{id:2,x:245,y:350},{id:3,x:fire.x+fire.width/2,y:fire.y+fire.height/2}]});
    await expect.poll(()=>Math.hypot(p.x-before.x,p.z-before.z)).toBeGreaterThan(1);
    await expect.poll(()=>p.ammo).toBeLessThan(30);expect(p.yaw).not.toBe(before.yaw);
...
test('loadout, duel mode and local settings survive their intended boundaries',async({page})=>{
  await identify(page,'Loadout Guest');
  await page.getByLabel('Game mode',{exact:true}).selectOption('duel');await page.getByLabel('Primary weapon').selectOption('sniper');
  await page.getByRole('button',{name:'Settings',exact:true}).click();
  await page.getByLabel('Field of view',{exact:false}).press('End');
  for(let i=0;i<6;i++)await page.getByLabel('Field of view',{exact:false}).press('ArrowLeft');
  await page.getByRole('button',{name:'Done'}).click();
  await page.reload();await expect(page.getByRole('button',{name:'Create private room'})).toBeVisible();
  await page.getByRole('button',{name:'Settings',exact:true}).click();await expect(page.getByLabel('Field of view')).toHaveValue('104');await page.getByRole('button',{name:'Done'}).click();
  await page.getByLabel('Game mode',{exact:true}).selectOption('duel');await page.getByLabel('Primary weapon').selectOption('sniper');
  const {room}=await create(page);await expect.poll(()=>room.state.gameMode).toBe('duel');
  await expect.poll(()=>room.state.players.get(room.state.hostPlayerId)?.primaryWeapon).toBe('sniper');
