...
function visit(index, parent, included = false) {
  ...
  if (included && node.mesh !== undefined && /SM_Bld_|SM_Env_(Beach|Rock|Flat)/.test(node.name)) {
    for (const primitive of document.meshes[node.mesh].primitives) meshes.push({ primitive, matrix });
  }
...
function readAccessor(index) {
  ...
  const read = { 5126: 'readFloatLE', 5125: 'readUInt32LE', 5123: 'readUInt16LE', 5121: 'readUInt8' }[accessor.componentType];
  if (!bytes || !dimensions) throw new Error('Unsupported source accessor');
  return Array.from({ length: accessor.count }, (_, i) => Array.from({ length: dimensions }, (_, k) => binary[read]((view.byteOffset ?? 0) + (accessor.byteOffset ?? 0) + i * (view.byteStride ?? dimensions * bytes) + k * bytes)));
}
...
const groups = new Map(), collision = [];
for (const { primitive, matrix } of meshes) {
  let group = groups.get(primitive.material);
  if (!group) { group = { vertices: [], indices: [], lookup: new Map() }; groups.set(primitive.material, group); }
  ...
  const transformed = positions.map(p => {
    ...
    return [Math.round((v.x - center.x) * scale * 100), Math.round((v.y - seaLevel) * scale * 100), Math.round((v.z - center.z) * scale * 100)];
  });
  for (let i = 0; i < indices.length; i += 3) {
    ...
    for (const index of triangle) {
      ...
      if (mapped === undefined) { mapped = group.vertices.length / 8; group.lookup.set(key, mapped); group.vertices.push(...vertex); }
...
function bufferView(bytes, extra = {}) {
  ...
  const id = views.length; views.push({ buffer: 0, byteOffset: offset, byteLength: bytes.length, ...extra });
...
for (const [material, group] of groups) {
  ...
  const box = new Box3();
  for (let i = 0; i < group.vertices.length; i += 8) box.expandByPoint(new Vector3(...group.vertices.slice(i, i + 3)));
  const position = accessors.length;
  accessors.push({ bufferView: vertexView, byteOffset: 0, componentType: 5126, count: group.vertices.length / 8, type: 'VEC3', min: box.min.toArray(), max: box.max.toArray() });
  accessors.push({ bufferView: vertexView, byteOffset: 12, componentType: 5126, count: group.vertices.length / 8, type: 'VEC3' });
  accessors.push({ bufferView: vertexView, byteOffset: 24, componentType: 5126, count: group.vertices.length / 8, type: 'VEC2' });
  const index = accessors.length;
  accessors.push({ bufferView: bufferView(Buffer.from(new Uint32Array(group.indices).buffer)), componentType: 5125, count: group.indices.length, type: 'SCALAR' });
  primitives.push({ attributes: { POSITION: position, NORMAL: position + 1, TEXCOORD_0: position + 2 }, indices: index, material });
}
const images = document.images.map(img => {
  const view = document.bufferViews[img.bufferView];
  return { mimeType: img.mimeType, bufferView: bufferView(binary.subarray(view.byteOffset, view.byteOffset + view.byteLength)) };
});
const output = { asset: { version: '2.0', generator: 'prepare-island.mjs', extras: document.asset.extras }, scene: 0, scenes: [{ nodes: [0] }], nodes: [{ mesh: 0, name: 'Island Fort: 112 metre structural derivative' }], meshes: [{ primitives }], buffers: [{ byteLength: offset }], bufferViews: views, accessors, materials: document.materials, images, textures: document.textures, samplers: document.samplers, extensionsUsed: document.extensionsUsed };
const rawJson = Buffer.from(JSON.stringify(output));
const json = Buffer.concat([rawJson, Buffer.alloc((4 - rawJson.length % 4) % 4, 32)]);
const rawBin = Buffer.concat(chunks), bin = Buffer.concat([rawBin, Buffer.alloc((4 - rawBin.length % 4) % 4)]);
...
fs.writeFileSync('apps/client/src/world/generated/island-fort.glb', Buffer.concat([header, json, binHeader, bin]));
const encoded = Buffer.alloc(collision.length * 2);
collision.forEach((value, i) => encoded.writeInt16LE(value, i * 2));
fs.writeFileSync('packages/shared/src/simulation/island-data.ts', `// Generated by scripts/prepare-island.mjs. Centimetre-quantized triangle soup.\nexport const ISLAND_TRIANGLES_BASE64: string = '${encoded.toString('base64')}';\n`);
const report = { source, sha256: crypto.createHash('sha256').update(sourceBytes).digest('hex'), sourceAttribution: document.asset.extras, selectedSector: 'Fort', scale, seaLevel, sourceCenter: center, playableHalfExtent: 60, triangles: collision.length / 9, drawCalls: primitives.length, runtimeBytes: header.length + json.length + binHeader.length + bin.length, collisionBytes: encoded.length };
