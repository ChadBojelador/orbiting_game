...
export interface PlayerView {
  playerId: string; displayName: string; team: Team; isConnected: boolean; isBot: boolean;
  reconnectDeadline: number; x: number; y: number; z: number; yaw: number; pitch: number;
  velocityX: number; velocityZ: number; verticalVelocity: number; isGrounded: boolean;
  inputSequence: number; status: PlayerStatus; protectedUntil: number;
  isSliding: boolean; isCrouching: boolean; slideUntil: number; slideReadyAt: number;
  hp: number; kills: number; deaths: number; currentWeaponSlot: number;
  primaryWeapon: WeaponId; weaponId: WeaponId; ammo: number; reserveAmmo: number;
  reloadUntil: number; fireReadyAt: number; respawnAt: number; spawnGeneration: number;
  lastKillerId: string; lastDeathWeapon: WeaponId; ping: number;
}
...
export interface SeatReservation { name: string; roomId: string; processId: string; sessionId: string; publicAddress?: string }
export interface RoomReservation { inviteCode: string; seat: SeatReservation }
