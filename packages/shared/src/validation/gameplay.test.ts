...
it('rejects non-finite, excessive and forged input across FPS boundaries', () => {
  ...
  for (const input of [
    ...
    { sequence: 1, x: 0, z: 0, hp: 100 },
    { sequence: 1, x: 0, z: 0, slide: 'yes' },
  ])
    expect(isMoveInput(input)).toBe(false);
  for (const input of [
    { yaw: Infinity, pitch: 0 },
    { yaw: 0, pitch: 0, victimId: 'forged' },
    { yaw: 0, pitch: 2 },
    { yaw: 0, pitch: 0, isAds: 1 },
  ])
    expect(isShootIntent(input)).toBe(false);
  expect(isShootIntent({ yaw: 0, pitch: 0, isAds: true })).toBe(true);
  expect(isWeaponSwitchIntent({ slot: 2 })).toBe(true);
