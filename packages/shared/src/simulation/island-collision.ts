...
function decodeTriangles(encoded: string): Float32Array {
  ...
  for (const character of encoded) {
    if (character === '=') break;
    value = (value << 6) | alphabet.indexOf(character); bits += 6;
...
const cells: ReadonlyMap<string, readonly number[]> = (() => {
  ...
  for (let i = 0; i < triangles.length; i += 9) {
    const minX = Math.floor(Math.min(triangles[i]!, triangles[i + 3]!, triangles[i + 6]!) / CELL_SIZE);
    const maxX = Math.floor(Math.max(triangles[i]!, triangles[i + 3]!, triangles[i + 6]!) / CELL_SIZE);
    const minZ = Math.floor(Math.min(triangles[i + 2]!, triangles[i + 5]!, triangles[i + 8]!) / CELL_SIZE);
    const maxZ = Math.floor(Math.max(triangles[i + 2]!, triangles[i + 5]!, triangles[i + 8]!) / CELL_SIZE);
    for (let x = minX; x <= maxX; x++) for (let z = minZ; z <= maxZ; z++) {
      const cellKey = key(x, z), bucket = result.get(cellKey) ?? [];
      bucket.push(i); result.set(cellKey, bucket);
    }
  }
  return result;
})();
...
export function islandRayDistance(origin: SpatialPosition, direction: SpatialPosition, range: number): number {
  let nearest = range;
  if (direction.y < 0 && origin.y >= 0) nearest = Math.min(nearest, -origin.y / direction.y);
  const visited = new Set<string>(), tested = new Set<number>();
  // Half-cell samples plus adjacent cells cover diagonal crossings and cell edges.
  const steps = Math.max(1, Math.ceil(Math.hypot(direction.x, direction.z) * range / (CELL_SIZE / 2)));
  for (let step = 0; step <= steps; step++) {
    const t = range * step / steps;
    if (t > nearest + CELL_SIZE) break;
    const cx = Math.floor((origin.x + direction.x * t) / CELL_SIZE), cz = Math.floor((origin.z + direction.z * t) / CELL_SIZE);
    for (let x = cx - 1; x <= cx + 1; x++) for (let z = cz - 1; z <= cz + 1; z++) {
      const cellKey = key(x, z); if (visited.has(cellKey)) continue; visited.add(cellKey);
      for (const triangle of cells.get(cellKey) ?? []) {
        if (tested.has(triangle)) continue; tested.add(triangle);
...
export function islandHeightAt(position: Position, maximum = 30): number {
  const top = Math.min(30, maximum), range = Math.max(0, top);
  const origin = { ...position, y: top }, direction = { x: 0, y: -1, z: 0 };
  let nearest = range;
  for (const triangle of cells.get(key(Math.floor(position.x / CELL_SIZE), Math.floor(position.z / CELL_SIZE))) ?? []) {
    const hit = intersect(triangle, origin, direction);
    if (hit !== null && hit < nearest) nearest = hit;
  }
...
export function isIslandBodyClear(position: Position, y: number, height: number, radius: number): boolean {
  for (const level of [y + 0.33, y + height / 2, y + height - 0.05]) {
    if (islandRayDistance({ x: position.x - radius, y: level, z: position.z }, { x: 1, y: 0, z: 0 }, radius * 2) < radius * 2) return false;
    if (islandRayDistance({ x: position.x, y: level, z: position.z - radius }, { x: 0, y: 0, z: 1 }, radius * 2) < radius * 2) return false;
  }
  const ceiling = islandRayDistance({ ...position, y: y + 0.33 }, { x: 0, y: 1, z: 0 }, Math.max(0, height - 0.33));
...
export function findIslandSpawns(): readonly Position[] {
  ...
  for (let x = -48; x <= 48; x += 4) for (let z = -48; z <= 48; z += 4) {
    ...
    if ([{x:x-0.7,z},{x:x+0.7,z},{x,z:z-0.7},{x,z:z+0.7}].some(q => Math.abs(islandHeightAt(q) - y) > 0.12)) continue;
    candidates.push(p);
  }
  ...
  while (chosen.length < 16 && candidates.length) {
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      const distance = chosen.length ? Math.min(...chosen.map(q => (p.x-q.x)**2+(p.z-q.z)**2)) : p.x*p.x+p.z*p.z;
