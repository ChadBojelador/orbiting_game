...
The server runs at 20 Hz. Sequenced movement is bounded and queued; message volume cannot purchase time. Server and prediction share 50 ms movement including velocity/friction, air control, jump, slide deadlines, crouch, sprint, and surface modifiers. Static collision uses substeps against map-specific shared geometry. Frostline uses analytic blocks/ramps. Island uses centimetre-quantized triangles derived from its rendered GLB, indexed into immutable four-metre X/Z cells. Height queries are bounded by current feet height so players can walk beneath roofs; body clearance and ceiling rays prevent walking/jumping through walls and ceilings. Players do not block one another.
...
Spawns score enemy distance, visibility, and recent death location across 16 points per map: authored for Frostline and generated offline from flat, clear Island surfaces with a two-metre exit in the initial facing direction. Island grounding samples the player's footprint to prevent sinking into adjacent ledges, and standing under low ceilings remains crouched until clear. Reserved disconnected bodies remain vulnerable but cannot act or respawn before reconnecting. Firing removes protection. Input queues and speculative movement clear across death/respawn and disconnect.
...
`npm run assets:island` extracts the Fort sector, removes dense foliage/tiny props, bakes transforms with uniform scale, quantizes vertices to centimetres, merges surfaces by material, and generates collision plus safe spawns. The 136,646,100-byte source remains unchanged. The runtime derivative is 4,937,100 bytes / 61,396 triangles / one material draw call. Both render and collision use the baked coordinates; neither uses browser-computed bounds. Source metadata and build hash live in `assets/island-build-report.json`. Runtime loads only the optimized GLB; movement is disabled and loading/error UI shown until it is ready. Textures and geometry are disposed on exit. The closed Frostline house uses a documented conservative shared box matching its normalized outer bounds.

The alternative of loading the complete archipelago would retain thousands of draw calls and oversize geometry, unsuitable for the mobile requirement. A separate physics dependency was unnecessary for this static prototype. Exact triangle hitscan plus sampled body clearance is an explicit compromise; narrow-edge/ceiling traversal and physical-device performance need playtesting. Island water is a walkable shallow surface at Y=0, with visible perimeter markers.
