...
it('applies only pre-deadline steps and locks gameplay before time-limit resolution',()=>{
  const state=new LobbyState(),p=new PlayerState();p.playerId='p';p.team='none';state.players.set('p',p);
  const gameplay=new GameplayController(state),match=new MatchController(state,()=>{},now=>gameplay.start(now));
  match.start(1000);Object.assign(p,{x:0,y:0,z:-34});state.phaseDeadline=1100;
  ...
  expect(p.inputSequence).toBe(1);expect(p.x).toBeGreaterThan(0);expect(state.phase).toBe('finished');
