import {
  GAMEPLAY, WEAPONS, simulateMovement, terrainHeightAt, isMoveInput, isShootIntent,
  isReloadIntent,isWeaponSwitchIntent, type GameplayEvent,type GameplayMessages,type MoveInput,
...
export class GameplayController {
  ...
  private readonly budgets=new Map<string,{moves:number;actions:number;resetsAt:number}>();
  private lastTick=0;private hasStarted=false;
  constructor(private readonly state:LobbyState,private readonly emit:(event:GameplayEvent)=>void=()=>{}){}
  start(now:number):void {
    this.hasStarted=true;this.lastTick=now;this.inputs.clear();
    for(const p of this.state.players.values())p.status='spectator';
    for(const p of this.state.players.values())if(p.isConnected && p.team!=='unassigned')this.respawn(p,now,false);
  }
  advance(now:number):void {
    if(!this.hasStarted || now<this.lastTick)return;
    this.lastTick=Math.max(this.lastTick,now-250);
    while(now-this.lastTick>=GAMEPLAY.tickMs){this.lastTick+=GAMEPLAY.tickMs;this.step(this.lastTick);}
  }
  handle(id:string,type:keyof GameplayMessages,payload:unknown,now:number):string|null {
    ...
    if(!budget || now>=budget.resetsAt){budget={moves:0,actions:0,resetsAt:now+1000};this.budgets.set(id,budget);}
    if(type==='input/move'?++budget.moves>30:++budget.actions>25)return 'Too many gameplay requests';
    ...
    if(type==='input/move'){
      ...
      if(player.status!=='alive'){player.inputSequence=payload.sequence;return null;}
      const queue=this.inputs.get(id)??[];
      if(queue.length>=GAMEPLAY.maxInputQueue)queue.shift();
      queue.push({input:payload,receivedAt:now});this.inputs.set(id,queue);return null;
    }
    this.advance(now);
    if(player.status!=='alive')return 'Player is not alive';
    if(type==='action/shoot'){
      if(!isShootIntent(payload))return 'Invalid shot intent';
      const error=this.weapons.fire(player,now);if(error)return error;
      player.yaw=payload.yaw;player.pitch=payload.pitch;
    ...
    if(type==='action/reload')return isReloadIntent(payload)?this.weapons.reload(player,now):'Invalid reload request';
    if(type==='action/switch-weapon'){
      if(!isWeaponSwitchIntent(payload))return 'Invalid weapon slot';
      this.weapons.switch(player,payload.slot);return null;
    }
  ...
  disconnect(id:string):void {
    ...
    if(p){p.velocityX=0;p.velocityZ=0;p.isSliding=false;}
  }
  private canPlay(now:number):boolean{return this.state.phase==='playing' && now<this.state.phaseDeadline;}
  private step(now:number):void {
    ...
    for(const p of this.state.players.values()){
      ...
      let pending=queue?.[0];
      while(pending && now-pending.receivedAt>GAMEPLAY.inputExpiryMs){p.inputSequence=pending.input.sequence;queue!.shift();pending=queue?.[0];}
