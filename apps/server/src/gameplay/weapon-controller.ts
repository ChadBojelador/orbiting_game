...
interface SlotState { id: WeaponId; ammo: number; reserve: number }
export class WeaponController {
  ...
  reset(player: PlayerState): void {
    ...
    this.inventories.set(player.playerId, ids.map(id => ({id, ammo:WEAPONS[id].magazineSize,reserve:WEAPONS[id].reserveAmmo})));
    player.currentWeaponSlot=0;player.reloadUntil=0;player.fireReadyAt=0;
    this.sync(player);
  }
  tick(player: PlayerState, now:number):void {
    ...
    slot.ammo+=needed;slot.reserve-=needed;player.reloadUntil=0;this.sync(player);
  }
  reload(player:PlayerState,now:number):string|null {
    this.tick(player,now);
    const slot=this.slot(player),stats=WEAPONS[slot.id];
    if(stats.slot==='melee' || player.reloadUntil || slot.ammo>=stats.magazineSize || !slot.reserve)return 'Cannot reload';
    player.reloadUntil=now+stats.reloadMs;return null;
  }
  switch(player:PlayerState,slot:number):void {
    player.reloadUntil=0;player.currentWeaponSlot=slot;this.sync(player);
  }
  ...
  private sync(player:PlayerState):void {
    const slot=this.slot(player);
    player.weaponId=slot.id;player.ammo=slot.ammo;player.reserveAmmo=slot.reserve;
  }
}
