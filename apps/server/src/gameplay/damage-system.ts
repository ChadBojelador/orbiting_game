...
export function fireHitscan(
  ...
  for (let pellet = 0; pellet < stats.pelletsPerShot; pellet++) {
    ...
    const direction = lookDirection(
      intent.yaw + (random() * 2 - 1) * spread,
      intent.pitch + (random() * 2 - 1) * spread,
    );
    ...
    for (const player of state.players.values()) {
      ...
      const hit = rayBox(
        ...
        {
          x: player.x + GAMEPLAY.playerRadius,
          y: player.y + bodyHeight(player),
          z: player.z + GAMEPLAY.playerRadius,
        },
        distance,
      );
    ...
    if (
      !victim ||
      victim.protectedUntil > now ||
      (state.gameMode === 'tdm' && victim.team === attacker.team)
    )
      continue;
    const isHeadshot = end.y >= victim.y + bodyHeight(victim) * (1 - GAMEPLAY.headHitboxRatio);
    const falloff =
      stats.falloffEnd > stats.falloffStart
        ? Math.max(
            0,
            Math.min(1, (distance - stats.falloffStart) / (stats.falloffEnd - stats.falloffStart)),
          )
        : 0;
    const damage = Math.min(
      ...
      Math.round(
        stats.damage *
          (1 - falloff * (1 - stats.falloffMinDamage)) *
          (isHeadshot ? stats.headshotMultiplier : 1),
      ),
    );
    victim.hp -= damage;
    victim.velocityX += direction.x * 1.2;
    victim.velocityZ += direction.z * 1.2;
    emit({
      type: 'player/hit',
      payload: {
    ...
    victim.status = 'dead';
    victim.deaths++;
    victim.respawnAt = now + GAMEPLAY.respawnDelayMs;
    victim.lastKillerId = attacker.playerId;
    victim.lastDeathWeapon = attacker.weaponId;
    victim.isSliding = false;
    victim.reloadUntil = 0;
    victim.velocityX = 0;
    victim.velocityZ = 0;
    attacker.kills++;
    if (state.gameMode === 'tdm') {
      if (attacker.team === 'ice') state.iceScore++;
      else state.waterScore++;
    }
    emit({
      type: 'player/killed',
      payload: {
