import { distanceSquared, hasGameplayLineOfSight, spawnPointsForMap, terrainHeightAt, type Position } from '@ice-water/shared';
...
export function selectSpawn(state:LobbyState,player:PlayerState,lastDeath?:Position):Position {
  ...
  for(const spawn of spawnPoints){
    ...
    for(const other of state.players.values()){
      ...
      score=Math.min(score,distance-(hasGameplayLineOfSight(eye,{x:other.x,y:other.y+1.6,z:other.z},state.mapId)?350:0));
    }
