...
export class GameSession {
  ...
  constructor(
    ...
    const update = () => {
      ...
      for (const p of this.view.players) {
        if (p.playerId === playerId) {
          if (p.spawnGeneration !== this.lastGeneration) {
            this.nextShotAt = 0;
            this.lastGeneration = p.spawnGeneration;
            this.input.cameraYaw = p.yaw;
            this.input.cameraPitch = p.pitch;
            this.input.reset();
          }
          this.prediction.reconcile(p, this.canMove(p));
        } else {
          let remote = this.remotes.get(p.playerId);
          if (!remote) {
            remote = new RemoteInterpolation();
            this.remotes.set(p.playerId, remote);
          }
    ...
    this.cleanups.push(
      () => room.onDrop.remove(drop),
      () => room.onReconnect.remove(reconnect),
    );
    ...
    subscribe('weapon/fired');
    subscribe('player/hit');
    subscribe('player/killed');
    subscribe('player/respawned');
    this.cleanups.push(
      room.onMessage<{ sentAt: number }>('session/pong', (p) => {
        this.ping = Math.max(0, Math.round(performance.now() - p.sentAt));
      }),
    );
    update();
  ...
  local(): PlayerView | undefined {
    return this.view.players.find((p) => p.playerId === this.playerId);
  }
  canMove(p: PlayerView): boolean {
    return (
      this.isConnected &&
      p.isConnected &&
      p.status === 'alive' &&
  ...
  private tick(): void {
    const p = this.local();
    if (!p || !this.isConnected || this.view.phase !== 'playing') return;
    ...
    if (!active) return;
    if (sample.slot !== undefined) {
      this.room.send('action/switch-weapon', { slot: sample.slot });
      return;
    }
    if (sample.hasReload) {
      this.room.send('action/reload', {});
      return;
    }
    ...
    if (
      wantsShot &&
      now >= Math.max(this.nextShotAt, p.fireReadyAt) &&
      !p.reloadUntil &&
      p.ammo > 0
    ) {
