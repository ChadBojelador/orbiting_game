import {
  GAMEPLAY, canRescueInPhase, distanceSquared, hasLineOfSight, isPlayPhase,
  type LobbyView, type PlayerView,
...
export function nearestTarget(view: LobbyView, local: PlayerView, now: number): PlayerView | undefined {
  const range = local.team === 'ice' ? GAMEPLAY.tagRange : GAMEPLAY.rescueRange;
  if (local.status !== 'active') return undefined;
  return view.players.filter(player => player.playerId !== local.playerId && player.team === 'water' &&
    (local.team === 'ice' ? player.status === 'active' && now >= player.protectedUntil : player.status === 'frozen') &&
    distanceSquared(local, player) <= range * range && hasLineOfSight(local, player))
...
export class GameSession {
    ...
    const update = () => {
      ...
      for (const player of this.view.players) {
        if (player.playerId === playerId) this.prediction.reconcile(player, this.canMove(player));
          ...
          if (!motion) { motion = new RemoteInterpolation(); this.remotes.set(player.playerId, motion); }
          motion.push(player, this.view.serverTime);
        }
      }
    };
    this.cleanups.push(room.onStateChange(update));
    this.cleanups.push(room.onDrop(() => { this.isConnected = false; this.input.reset(); this.prediction.reset(); this.rescueTarget = ''; }));
    this.cleanups.push(room.onReconnect(() => { this.isConnected = true; this.input.reset(); this.prediction.reset(); update(); }));
    update();
  ...
  serverNow(): number { return this.view.serverTime + performance.now() - this.receivedAt; }
  local(): PlayerView | undefined { return this.view.players.find(player => player.playerId === this.playerId); }
  canMove(player: PlayerView): boolean {
    return this.isConnected && player.isConnected && player.status === 'active' && isPlayPhase(this.view.phase) &&
      (this.view.phaseDeadline === 0 || this.serverNow() < this.view.phaseDeadline);
  }
  destroy(): void { for (const cleanup of this.cleanups) cleanup(); this.stopRescue(); }
  ...
  private tick(): void {
    ...
    const canRescue = canMove && input.isRescuing && local.team === 'water' && target &&
      canRescueInPhase(this.view.phase, this.view.phaseDeadline, now);
    if (!canRescue) { this.stopRescue(); return; }
