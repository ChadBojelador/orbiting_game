...
export class LocalPrediction {
  ...
  reconcile(player:PlayerView,canMove:boolean):void {
    ...
    if(player.spawnGeneration!==this.generation){this.pending=[];this.generation=player.spawnGeneration;}
...
export interface PresentationMotion extends Position {y:number;yaw:number}
export class LocalPresentation {
  ...
  update(target:PresentationMotion,seconds:number,shouldSnap=false):PresentationMotion {
    if(!this.current || shouldSnap || Math.hypot(target.x-this.current.x,target.y-this.current.y,target.z-this.current.z)>5)this.current={...target};
    else{
      const alpha=1-Math.exp(-24*seconds);
      this.current.x+=(target.x-this.current.x)*alpha;this.current.y+=(target.y-this.current.y)*alpha;this.current.z+=(target.z-this.current.z)*alpha;
      this.current.yaw+=Math.atan2(Math.sin(target.yaw-this.current.yaw),Math.cos(target.yaw-this.current.yaw))*alpha;
    }
    return {...this.current};
  }
...
export class RemoteInterpolation {
  ...
  push(player:PlayerView,time:number):void {
    ...
    if(previous?.generation!==player.spawnGeneration || player.status!=='alive')this.samples=[];
    this.samples.push({x:player.x,y:player.y,z:player.z,yaw:player.yaw,time,generation:player.spawnGeneration});
  ...
  at(time:number):PresentationMotion|undefined {
    for(let i=1;i<this.samples.length;i++){
      ...
      return {x:a.x+(b.x-a.x)*t,y:a.y+(b.y-a.y)*t,z:a.z+(b.z-a.z)*t,yaw:a.yaw+Math.atan2(Math.sin(b.yaw-a.yaw),Math.cos(b.yaw-a.yaw))*t};
    }
