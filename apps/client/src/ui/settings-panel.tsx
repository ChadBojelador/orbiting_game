...
export function SettingsPanel({value,onChange,onClose}:{value:FpsSettings;onChange:(v:FpsSettings)=>void;onClose:()=>void}){
  return <section className="settings-panel" role="dialog" aria-label="Settings" aria-modal="true">
    <h2>Settings</h2>
    <label>Look sensitivity <output>{value.sensitivity.toFixed(1)}</output><input type="range" min="0.2" max="3" step="0.1" value={value.sensitivity} onChange={e=>onChange({...value,sensitivity:Number(e.target.value)})}/></label>
    <label>Field of view <output>{value.fov}°</output><input type="range" min="90" max="110" value={value.fov} onChange={e=>onChange({...value,fov:Number(e.target.value)})}/></label>
    <label>Volume <output>{Math.round(value.volume*100)}%</output><input type="range" min="0" max="1" step="0.05" value={value.volume} onChange={e=>onChange({...value,volume:Number(e.target.value)})}/></label>
