...
export function App(){
  ...
  useEffect(()=>{if(isComplete){document.exitPointerLock();sceneRef.current?.getInput().reset();}},[isComplete]);
  useEffect(()=>{
    let active=true;
    void reconnectRoom().then(next=>{if(next&&active)attach(next);}).catch(reason=>{if(active)setError(reason instanceof Error?reason.message:'Unable to reconnect');}).finally(()=>{if(active)setBusy(false);});
    const timer=window.setInterval(()=>setNow(clock.current.now()),100);
    return ()=>{active=false;window.clearInterval(timer);roomCleanup.current.forEach(cleanup=>cleanup());};
  },[]);
  useEffect(()=>{const audio=lobbyAudio.current;return()=>audio?.destroy();},[]);
  useEffect(()=>{
  ...
  useEffect(()=>{
    saveSettings(settings);if(scene)scene.settings=settings;
    lobbyAudio.current?.setSettings(settings);lobbyAudio.current?.setActive(!isInGame);
  ...
  function attach(next:LobbyRoom){
    roomCleanup.current.forEach(cleanup=>cleanup());roomCleanup.current=[];roomRef.current=next;setRoom(next);setError('');setFeed([]);setConnection('Connected');
    const update=()=>{if(!next.state?.players)return;const current=snapshot(next.state);setView(current);clock.current.update(current.serverTime);setNow(clock.current.now());};
    next.onStateChange(update);roomCleanup.current.push(()=>next.onStateChange.remove(update));
    roomCleanup.current.push(next.onMessage<SessionError>('session/error',message=>setError(message.message)));
    ...
    roomCleanup.current.push(next.onMessage<GameplayEvents['player/killed']>('player/killed',message=>setFeed(old=>[...old.slice(-4),{...message,key:++feedKey.current}])));
    roomCleanup.current.push(next.onMessage<GameplayEvents['player/hit']>('player/hit',message=>{
      const localId=readGuest()?.playerId;
      if(message.attackerId===localId)setHit({until:message.serverTime+200,headshot:message.isHeadshot});
      if(message.victimId===localId){
        const players=[...next.state.players.values()],attacker=players.find(player=>player.playerId===message.attackerId),victim=players.find(player=>player.playerId===message.victimId);
        const yaw=sceneRef.current?.getInput().cameraYaw??0,angle=attacker&&victim?Math.atan2(attacker.x-victim.x,-(attacker.z-victim.z))+yaw:0;
    ...
    next.onLeave(()=>{
      ...
      roomRef.current=null;clearReconnect();setRoom(null);setView(null);setError(complete?'':'Your room connection ended. Create a room or join again.');
    });
    update();
  }

  async function run(action:()=>Promise<void>){setError('');setBusy(true);try{await action();}catch(reason){setError(reason instanceof Error?reason.message:'Unable to complete this action. Try again.');}finally{setBusy(false);}}
  function identify(name:string){const value=sanitizeDisplayName(name);if(!value){setError('Choose a name with 2–20 letters or numbers.');return;}void run(async()=>setGuest(await createGuest(value)));}
  async function create(mode:GameMode,mapId:MapId,primary:WeaponId){if(!guest)return;const next=await reserveRoom(guest);attach(next);next.send('room/configure',{gameMode:mode,mapId});next.send('player/loadout',{primaryWeapon:primary});}
  function join(code:string,primary:WeaponId){const invite=normalizeInviteCode(code);if(!invite){setError('Enter the eight-character invite code from your host.');return;}if(guest)void run(async()=>{const next=await reserveRoom(guest,invite);attach(next);next.send('player/loadout',{primaryWeapon:primary});});}
  ...
  return <LobbyScreen guest={guest} room={room} view={view} error={error} isBusy={isBusy} now={now} settings={settings} onSettings={setSettings} onIdentify={identify} onCreate={(mode,mapId,weapon)=>void run(()=>create(mode,mapId,weapon))} onJoin={join} onLeave={()=>void run(leave)} onForgetGuest={()=>{forgetGuest();setGuest(null);setError('');}} onAudioUnlock={()=>void lobbyAudio.current?.unlock()} onUiCue={cue=>lobbyAudio.current?.cue(cue)}/>;
}
