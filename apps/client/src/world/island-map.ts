...
export class IslandMap {
  ...
  constructor(scene: Scene) {
    ...
    const sea = new Mesh(
      new BoxGeometry(120, 0.3, 120),
      new MeshStandardMaterial({ color: 0x48b3c4, roughness: 0.35 }),
    );
    sea.position.y = -0.15;
    this.group.add(sea);
    const boundaryMaterial = new MeshStandardMaterial({ color: 0xe0cf94 });
    for (const side of [-1, 1]) {
      const north = new Mesh(new BoxGeometry(120, 0.25, 0.25), boundaryMaterial);
      north.position.set(0, 0.1, side * 60);
      const east = new Mesh(new BoxGeometry(0.25, 0.25, 120), boundaryMaterial);
      east.position.set(side * 60, 0.1, 0);
