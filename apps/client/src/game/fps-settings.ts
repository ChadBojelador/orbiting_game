  ...
  return {
    ...
    crosshair:
      typeof v.crosshair === 'string' && /^#[0-9a-f]{6}$/i.test(v.crosshair)
        ? v.crosshair
        : '#ffffff',
    reducedEffects: v.reducedEffects === true,
    controls: v.controls === 'touch' || v.controls === 'mouse' ? v.controls : 'auto',
  };
}
export function readSettings(): FpsSettings {
  try {
    return parseSettings(JSON.parse(localStorage.getItem('ice-water/fps-settings') ?? 'null'));
  } catch {
    return { ...DEFAULT_SETTINGS };
  }
}
export function saveSettings(value: FpsSettings): void {
  try {
    localStorage.setItem('ice-water/fps-settings', JSON.stringify(value));
  } catch {
