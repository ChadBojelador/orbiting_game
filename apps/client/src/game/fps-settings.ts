export interface FpsSettings { sensitivity:number; fov:number; volume:number; crosshair:string; reducedEffects:boolean }
export const DEFAULT_SETTINGS:FpsSettings={sensitivity:1,fov:96,volume:0.35,crosshair:'#ffffff',reducedEffects:false};
export function parseSettings(value:unknown):FpsSettings {
  ...
  return {sensitivity:number('sensitivity',0.2,3,1),fov:number('fov',90,110,96),volume:number('volume',0,1,0.35),
    crosshair:typeof v.crosshair==='string' && /^#[0-9a-f]{6}$/i.test(v.crosshair)?v.crosshair:'#ffffff',reducedEffects:v.reducedEffects===true};
}
