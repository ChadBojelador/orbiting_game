...
it('sanitizes persisted settings against invalid numbers and CSS values',()=>{
  expect(parseSettings({fov:999,sensitivity:NaN,volume:-1,musicVolume:9,sfxVolume:-2,isMuted:true,crosshair:'url(example)',reducedEffects:true})).toEqual({fov:110,sensitivity:1,volume:0,musicVolume:1,sfxVolume:0,isMuted:true,crosshair:'#ffffff',reducedEffects:true,controls:'auto'});
});
