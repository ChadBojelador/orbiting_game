...
it('sanitizes persisted settings against invalid numbers and CSS values',()=>{
  expect(parseSettings({fov:999,sensitivity:NaN,volume:-1,crosshair:'url(example)',reducedEffects:true})).toEqual({fov:110,sensitivity:1,volume:0,crosshair:'#ffffff',reducedEffects:true});
});
