import { BufferGeometry,Float32BufferAttribute,Line,LineBasicMaterial,Mesh,BoxGeometry,MeshBasicMaterial,type Scene } from 'three';
...
export class HitEffects {
  ...
  private readonly chip=new BoxGeometry(.04,.04,.04);
  ...
  shot(origin:SpatialPosition,end:SpatialPosition,now:number):void {
    ...
    for(let i=0;i<3;i++){
      const mesh=new Mesh(this.chip,this.chipMaterial);mesh.position.set(end.x,end.y,end.z);this.scene.add(mesh);
      this.impacts.push({mesh,until:now+220,dx:(Math.random()-.5)*.05,dy:Math.random()*.04,dz:(Math.random()-.5)*.05});
    }
    while(this.impacts.length>144)this.impacts.shift()!.mesh.removeFromParent();
  }
  update(now:number):void{
    for(let i=this.effects.length-1;i>=0;i--)if(now>=this.effects[i]!.until)this.remove(i);
    for(let i=this.impacts.length-1;i>=0;i--){const p=this.impacts[i]!;if(now>=p.until){p.mesh.removeFromParent();this.impacts.splice(i,1);}else{p.mesh.position.x+=p.dx;p.mesh.position.y+=p.dy;p.mesh.position.z+=p.dz;p.dy-=.004;}}
  }
  destroy():void{while(this.effects.length)this.remove(0);this.impacts.forEach(p=>p.mesh.removeFromParent());this.chip.dispose();this.chipMaterial.dispose();}
