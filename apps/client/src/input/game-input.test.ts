...
it('normalizes keyboard/touch axes and maps movement to camera forward',()=>{
  ...
  expect(touchAxes(1,1,45)).toEqual({x:0,z:0});expect(touchAxes(90,0,45)).toEqual({x:1,z:0});
});
it('clamps mouse look and clears one-shot and held input on reset',()=>{
  const input=new GameInput();input.look(100000,100000);expect(Math.abs(input.cameraPitch)).toBeLessThan(Math.PI/2);
  input.pressJump();input.pressSlide();input.pressFire();input.pressReload();
  expect(input.sample()).toMatchObject({jump:true,slide:true,hasShot:true,hasReload:true});
  expect(input.sample()).toMatchObject({jump:false,slide:false,hasShot:false,isFiring:true});
  input.reset();expect(input.sample().isFiring).toBe(false);
});
